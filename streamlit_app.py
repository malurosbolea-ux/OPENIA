...
st.set_page_config(
    page_title="OpinIA", page_icon="💬", layout="wide",
...
<style>
    ...
    .metric-label { font-size: 0.85rem; color: #7B8794; margin-top: 4px; }
    .section-tag { font-size: 0.8rem; font-weight: 600; color: #5B9BD5; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 4px; }
    ...
    .sentiment-pos { color: #2B6CB0; font-weight: 700; }
    .sentiment-neg { color: #C53030; font-weight: 700; }
    .sentiment-neu { color: #7b6fa0; font-weight: 700; }
    .tag { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; margin: 2px; }
...
def analizar_sentimiento(texto):
    try:
        return client.text_classification(texto[:512], model="nlptown/bert-base-multilingual-uncased-sentiment")
    except Exception as e:
        st.error(f"El modelo está cargando, espera unos segundos y vuelve a intentarlo. ({e})")
        return None

def clasificar_zero_shot(texto, categorias):
    try:
        return client.zero_shot_classification(texto[:512], categorias, multi_label=True)
    except Exception as e:
        st.error(f"El modelo está cargando, espera unos segundos y vuelve a intentarlo. ({e})")
        return None
...
def estrellas_a_sentimiento(label):
    n = int(label[0])
    if n <= 2:
        return "negativo", n, "sentiment-neg"
    elif n == 3:
        return "neutro", n, "sentiment-neu"
    else:
        return "positivo", n, "sentiment-pos"
...
if pagina == "Analizar reseña":
    ...
    st.markdown("Escribe o pega cualquier opinión de producto en español. El modelo **BERT multilingüe** detecta automáticamente si es positiva, negativa o neutra.")
    st.markdown("")
    ...
    col_ej, col_input = st.columns([1, 2])
    with col_ej:
        st.markdown("**Prueba un ejemplo:**")
        ejemplo_elegido = st.selectbox("Ejemplos", ejemplos, label_visibility="collapsed")
        usar_ejemplo = st.button("Usar este ejemplo", use_container_width=True)
    with col_input:
        default_text = ejemplo_elegido if usar_ejemplo else ""
        texto_usuario = st.text_area("Escribe la reseña aquí:", value=default_text, height=120, placeholder="Ej: Las zapatillas son cómodas pero la suela se despega al mes...")
    ...
    if st.button("🔍  Analizar sentimiento", type="primary", use_container_width=True):
        if not texto_usuario.strip():
            ...
            if resultado:
                top = resultado[0]
                sentimiento, estrellas, css_class = estrellas_a_sentimiento(top["label"])
                st.markdown("")
                c1, c2, c3 = st.columns(3)
                with c1:
                    emoji = "😊" if sentimiento == "positivo" else "😐" if sentimiento == "neutro" else "😠"
                    st.markdown(f'<div class="metric-card"><div style="font-size:2.5rem">{emoji}</div><div class="metric-number"><span class="{css_class}">{sentimiento.upper()}</span></div><div class="metric-label">Sentimiento detectado</div></div>', unsafe_allow_html=True)
                with c2:
                    st.markdown(f'<div class="metric-card"><div class="metric-number">{estrellas}★</div><div class="metric-label">Estrellas predichas</div></div>', unsafe_allow_html=True)
                with c3:
                    st.markdown(f'<div class="metric-card"><div class="metric-number">{top["score"]*100:.0f}%</div><div class="metric-label">Confianza del modelo</div></div>', unsafe_allow_html=True)
                ...
                for r in resultado:
                    ...
                    st.markdown(f'<div style="display:flex;align-items:center;gap:10px;margin:4px 0"><span style="min-width:50px;font-weight:600">{n_s}★</span><div style="flex:1;height:24px;background:#EEF2F7;border-radius:12px;overflow:hidden"><div style="width:{pct}%;height:100%;background:{bar_c};border-radius:12px"></div></div><span style="min-width:50px;font-size:0.9rem">{pct:.1f}%</span></div>', unsafe_allow_html=True)
                st.markdown('<div class="quote-box" style="margin-top:16px">Modelo: <strong>nlptown/bert-base-multilingual-uncased-sentiment</strong> (110M parámetros)</div>', unsafe_allow_html=True)
    ...
    if st.button("🏷️  Clasificar", type="primary", use_container_width=True):
        ...
        if not texto_zs.strip():
            ...
            with st.spinner("Clasificando con BART (zero-shot)..."):
                resultado = clasificar_zero_shot(texto_zs, categorias)
            if resultado:
                st.markdown("")
                ...
                for label, score in zip(resultado["labels"], resultado["scores"]):
                    ...
                    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin:6px 0"><div style="flex:1;max-width:500px;height:28px;background:#EEF2F7;border-radius:14px;overflow:hidden"><div style="width:{pct}%;height:100%;background:{color};border-radius:14px;display:flex;align-items:center;padding-left:12px"><span style="color:white;font-size:0.8rem;font-weight:600">{pct:.0f}%</span></div></div><span style="font-size:0.95rem;min-width:250px">{label}</span></div>', unsafe_allow_html=True)
...
# ── RESULTADOS ──
    ...
    with tab3:
        ...
        for i, (nombre, palabras, color) in enumerate(temas):
            ...
            with col:
                st.markdown(f'<div class="result-box" style="border-left:4px solid {color}"><span style="color:{color};font-size:0.8rem;font-weight:600">TEMA {i+1}</span><div style="font-weight:600;margin:4px 0">{nombre}</div><span style="font-size:0.85rem;color:#7B8794">{palabras}</span></div>', unsafe_allow_html=True)
...
# ── SOBRE OPINIA ──
    ...
    st.markdown("")
    st.markdown("---")
    st.markdown('<p style="text-align:center;color:#7B8794;font-size:0.9rem">María Luisa Ros Bolea · malurosbolea@gmail.com · <a href="https://www.linkedin.com/in/mar%C3%ADa-luisa-ros-bolea-400780160/">LinkedIn</a> · <a href="https://malurosbolea-ux.github.io/digital-strategy-portfolio/">Portfolio</a></p>', unsafe_allow_html=True)
